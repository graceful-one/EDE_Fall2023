...
## Opening discussion
good way to get information about what plots to make. deciscion tree
...
* geom_line
* geom_area
* geom_abline (plus geom_hline and geom_vline)- horizontal and vertical lines
* geom_text
...
print(O3plot) #need to print to actually see it 

# Fix this code
#specifiy color inside aesthetics, makes a new layer instead of changing the color
...
# How could you automatically assign a marker color to a variable? if you just want to have it assign colors for you based on something do it inside aesthetics
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) +
  geom_point()
print(PMplot)

# Separate plot with facets,multiple plots for each site in this case, set of sub plots
PMplot.faceted <-
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# Scatterplot
#variables on both x and y 
#really hard to see what is happening due to outliers 
lightvsDO <- 
...
# Adjust axes
#set axis length so you can actually see
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) +
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() # changes 0 to be on top 
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #a line, lm= linear model a trend line
  geom_abline(aes(slope = 16, intercept = 0)) #when you have a line already
print(NvsP)
...
violin plots work better with large ammounts of data

```{r, fig.height = 5, fig.width = 6, warning = FALSE}
# Box and whiskers plot
Nutrientplot3 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"?- would just fill the boxes
print(Nutrientplot3) #box plot of each concentration seperated by which lake and which nutrient, kinda hard to read.

# Dot plot
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) #doesnt stack the dots, puts them to the side, binwidth decides how close to make them stack 
print(Nutrientplot4)

# Violin plot - shows density plot, still hard to see much with this particular data 
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# Frequency polygons
# Using a tidy dataset not the gathered 
# like a line graph for each of their frequencies
#some problems - no legend, x axis only has first number
# just generally bad 
Nutrientplot6 <-
...
#value of gathering, much better plot
Nutrientplot7 <-   
...
#way too many lines crossing 

# Ridgeline plot
#multiple different frequency plots one for each nutrient 
Nutrientplot6 <-
