...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #will make it so they dont show up at all
summary(NTL.nutrient.data.PeterPaul$lakename)
...
#needs to make two columns one with which nutrient it is and one with the value
#turns 4 nutrient columns into two 

#filter out rows where concentration is na because that adds no data 
...
#just revers of the last thing, define where names should be from and where values should be from
...
* `left_join`: return all rows from x, and all columns from x and y (mutating join). this just adds to rows in x that also have data in y
* `anti_join`: return all rows from x where there are *not* matching values in y, keeping just columns from x (filtering join).
* `full_join`: return all rows and all columns from x and y. Returns NA for missing values (mutating join). this keeps everything
...
#tells you what it joins by, what columns
#output file has all the common ones columns, and all the physical and all the chemical, not that many that have measruemnts at the same time 
...
#this brings together ones that already have the same columns but different rows, or muliple datasets measureing the same qualities
