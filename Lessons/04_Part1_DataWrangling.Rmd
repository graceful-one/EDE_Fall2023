...
> Add notes here: looking at lakes, chemical and physical data,temp o2 levels ect
first load in packages and the data
...
library(lubridate)
#importing data add "stringsAsFactors = TRUE" for reading csv
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE)

#taking a look at the data
colnames(NTL.phys.data) #some at same date but different depths
head(NTL.phys.data) 
summary(NTL.phys.data) 
str(NTL.phys.data) # find data type of each column 
dim(NTL.phys.data)

# fixing dates
class(NTL.phys.data$sampledate)
# Format sampledate as date
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y") #formats the dates to this new format and saves as dates 
```
...
Notes: make it simple, get what you want and reuasable
...
note: functions have names that fit well to what you would expect it to
```{r, results = "hide"}
vignette("dplyr") #shows basics of what the package does, shows which does things for rows and which for columns 
```
...
note: works on rows , based on logical operatiors, can also use & for and and | for or if there are multiple conditions. rows have to be of the same type 
...
#gets only the surface data 

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) # another way to do the same thing 
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25) #includes ones that arent quite on the surface but very close all th
...
#needs quotation marks cause it is a character, either paul or peter
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")
#can also do by saying not any of the others
...
#can do by checking inclusion in a list 
...
basically a sort function

```{r arranging data}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth)
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) #need to specify to do decesnding order, higher depth number first 
...
#can choose just relevant columns (full dataset, names of columns)
```
### Mutate
...
# adds a new column so you can just save to the same place 
...
#lubidate lets you just pull out month 
...
#just resave by selecting all the columns but in a different order

# find out the start and end dates of the dataset
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #if ordered by date you can calculate by putting in number of observations, returns the time interval 
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) #gets same result 
```
...
#make a list of steps to do %>% basically means then
#dont need to provide the dataset in all of the functions
...
be sure to save things as csv once it is tidy 
give path and file name 
