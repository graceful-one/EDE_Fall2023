...
#usually done making a date sequence of first to last date in data,
#sometimes just no row for it, then left join to the date sequence 
#this fills missing dates with nas
...
ggplot(BR_complete, aes(x=BR_complete$Date,y=BR_complete$Rainfall))+
  geom_line()+
  geom_smooth(method = "lm")

ggplot(BR_complete, aes(x=BR_complete$Date,y=BR_complete$Temperature_C))+
  geom_line()+
  geom_smooth(method = "lm")
...
BR_rain <- ts(data = BR_complete$Rainfall,frequency = 12,start=c(1901,1))
BR_temp <- ts(data = BR_complete$Temperature_C, frequency = 12,start=c(1901,1))
...
BR_rain_decop <- stl(BR_rain,s.window="periodic")
#if there is any pattern in remainder something went wrong
#gray boxes on the right, how much it zoomed in, bigger box had to zoom in a lot 
#that part is less important
