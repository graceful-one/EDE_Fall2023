...
matrix1[4, ] # idealy would be two numbers, this just calls a row 
matrix1[ , 3] # just calls a row 
matrix1[c(12, 14)] # calls element 12th position and 14th position 
matrix1[c(12:14)] # calls a set of elements 
matrix1[2:4, 1:3] # calls a subset of rows and a subset of columns
...
dataframe1 <- data.frame(d,e,f) # faster way to make a data frame of multiple vectors 
...
> Answer: shows the different parts of the data frame 
...
high_temp_f<-as.vector(c(81,82,84,84,77,72,79,77,78,78))
low_temp_f <- as.vector(c(53,55,58,59,61,54,62,63,62,62))
```
...
```{r DataExercise-1b}
high_temp_c <- as.vector((high_temp_f-32)*5/9)
low_temp_c <- as.vector((low_temp_f-32)*5/9)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
weather_forcast <- data.frame("high temp f"= high_temp_f,"low temp f"= low_temp_f,"high temp c"=high_temp_c,"low temp c"=low_temp_c) # other way to name them 
```
...
```{r DataExercise-1d}
summary(weather_forcast)
sd(weather_forcast$high.temp.f) #can only work on one column for a df 
...
```{r dateorigin}
#Check if "1970-01-01" is your origin date. this is what computer considers as beginning
lubridate::origin
